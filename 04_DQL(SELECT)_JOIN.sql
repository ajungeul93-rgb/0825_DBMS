...
-- 전체 사원들의 사번, 사원명, 직급 코드, 직급명을 한 번에 조회
SELECT 
       EMP_ID
     , EMP_NAME
     , JOB_CODE
 ...
 * --------------------------------------------------------------------------------------
 *               포괄 조인                 |       왼쪽 외부조인(LIFT OUTER JOIN)
 *             (LEFT OUTER)              |       오른쪽 외부조인(RIGHT OUTER JOIN)
 *             (RIGHT OUTER)             |    전체 외부조인(FULL OUTER JOIN) -> 오라클 X
 * --------------------------------------------------------------------------------------
 *              카데시안 곱                 |                교차 조인 
 ...
 * 
 *  JOIN 시 연결 시키는 컬럼의 값이 일치하는 행들만 조인되서 조회
 *  ( == 일치하지 않는 값들을 조회에서 제외)
 */
...
-- 사원명, 부서코드, 부서명
-- 연결 컬림이 컬럼명이 다름(EMPLOYEE - DEPT_CODE / DEPARTMENT - DEPT_ID)
-- 무조건 ON구문만 사용가능(USING은 못씀 불가능)
SELECT 
       EMP_NAME
     , DEPT_CODE
     , DEPT_TITLE
...
-- 2) RIGHT [ OUTER ] JOIN : 두 테이블 중 오른쪽에 기술한 테이블을 기준으로 JOIN
 ...
 * 
 *  두 테이블의 행들이 곱해진 조합 뽑아냄 => 데이터 많을 수록 방대한 행이 생겨남
 * => 과부하의 위험
 */
 ...
 * - INTERSERCT : 교집합( 두 쿼리문의 수행 결과값 중 중복된 부분)
 * - MINUS : 차집합 ( 선생 쿼리문 결과값 빼기 후에 쿼리문의 결과값을 한 결과)
 * 
...
-- UNION쓰려면 SELECT절에 기술하는 컬럼이 같아야함. (OR 쓰면됨 비슷함)
...
SELECT
 ...
 WHERE
       SALARY > 3000000;
-----------------------------------------------------------------------------------------------
-- 3. INTERSECT(교집합 두 쿼리문의 수행 결과값 중 중복된 부분)
SELECT
       EMP_NAME
     , DEPT_CODE
     , SALARY
...
SELECT
 ...
 WHERE
       SALARY > 3000000;
-----------------------------------------------------------------------------------------------
-- 4.MINUS (차집합 - 선행 쿼리문 결과값 빼기 후에 쿼리문의 결과값을 한 결과)
SELECT
       EMP_NAME
     , DEPT_CODE
     , SALARY
