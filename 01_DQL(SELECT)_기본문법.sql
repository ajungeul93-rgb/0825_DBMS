...
SELECT emp_name, email, salary FROM employee; -- 이렇게해도 돌아감
...
-- EMPLOYEE테이블로부터 사원명, 월급, 보너스, 보너스를 포함한 연봉
SELECT
       EMP_NAME
     , SALARY
     , BONUS
...
---------------------------------------------------------------------
/*
 * SELECT문을 이용해서 조회를 할 떄 조건을 부여하는 방법!
 * 
 * < WHERE 절 >
...
-- EMPLOYEE테이블로붙어 사원들의 사원명, 급여조회 --> 급여가 300만원 이상인 사원들만
...
-- EMPLOYEE 테이블에서 급여서 350미만이거나 500을 초과하는 사원들의 사번, 이름, 직급코드
SELECT 
       EMP_ID
     , EMP_NAME
     , JOB_CODE
...
-- BETWEEN AND 연사자는 DATE형식에도 사용 가능
...
/*
 * < LIKE '특정패턴' >
 * 비교하려는 컬럼의 값이 내가 지정한 특정 패턴에 만족할 경우조회
 * 
 * [ 표현법 ]
 * 비교대상컬럼 LIKE '특정패턴'
 * 
 ...
 * ORDER BY절을 작성하지 않으면 ResulSet은 정렬이 안된 상태
 * 
 * [ 표현법 ]
 * SELECT
 *        컬럼명
