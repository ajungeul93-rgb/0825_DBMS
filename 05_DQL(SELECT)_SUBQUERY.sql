...
----------------------------------------------------------------------------------------
-- 간단한 서브쿼리 예시 두 번째
-- 전체 사원의 평균 급여보다 더 많은 급여를 받고 있는 사원들의 사번, 서원명을 조회
 ...
 * 서브쿼리의 분류
 * 
 * 서브퀘리를 수행한 결과가 몇 행 몇 열이냐에 따라서 분류됨
 * 
...
--------------------------------------------------------------------------------------------------
-- 우리가 지금 하고 있는 작업이 무엇을 하고 있는 건가요???
-- DB에다가 주고받고 --> 컴퓨터 작업
------------------------------------------------------------------------------------------
/*
 * 2. 다중 행 서브쿼리
 ...
 * 
 * - IN(10, 20, 30) : 여러 개의 결과 값 중 한개라도 일치하는 값이 있다면
 */
...
-- 이승철 사원 또는 선승제 사원과 같은 부서인 사원들이 사원명, 핸드폰번호 조회
...
-- 과장직급인데 모든 차장 직급의 급여보다 더 많이 받는 직원
SELECT
       SALARY
  FROM
       EMPLOYEE
 ...
 *  3. 다중 열 서브쿼리
 * 
 * 조회 결과는 한 행이지만 나열된 컬럼의 수가 다수개일 때
 */
 ...
 * 
 * 주로 SELECT 절에 사용하는 쿼리를 의미(WHERE나 FROM이나 다 쓸순 있음)
...
-- * ROWNUM : 오라클에서 자체적으로 제공해주는 컬람, 조회된 순서대로 순번을 붙여줌
SELECT
       EMP_NAME
     , SALARY
     , ROWNUM
