...
-- EMPLOYEE 테이블로부터 사원명과 이메일컬럼과 EMALIL 컬럼에서의 아이디 값만 추출해서 조회
...
SELECT 
       EMP_NAME
     , CONCAT(FLOOR(SYSDATE - HIRE_DATE), '일') "근무 일자"
  FROM
       EMPLOYEE
 WHERE 
...
-- ADD_MONTHS(DATE, NUMBER) : 특정 날짜에 해당 숫자만큼의 개월 수를 더한 날짜(DATE로 반환)
SELECT 
...
SELECT 
     ...
     , NEXT_DAY(SYSDATE, '금')
     , NEXT_DAY(SYSDATE, 6) -- 1 : 일요일, 2 : 월요일, 3 : 화요일, 4 : 수요일, 5 : 목요일, 6 : 금요일, 7 : 토요일
  FROM
       DUAL;
...
SELECT 
     ...
     , TO_CHAR(1234, '99999') -- 자리수보다 큰 공강을 공백문자로 채움
...
SELECT 
       TO_CHAR(SYSDATE, 'DD')
     , TO_CHAR(SYSDATE, 'D') -- 1. 일, 2.월, 3. 화 해서 3
     , TO_CHAR(SYSDATE, 'DDD') -- 1월 1일로부터 얼마나 지났는지  
  FROM 
       DUAL;
 ...
 * < 선택 함수>
 *
 * DECODE(비교대상(컬럼명/산술연산/함수식), 조건값1, 결과값1, 조건값2, 결과값3 .......결과값X);
 * 
 *  - 자바에서의 switch문과 유사
 *  switch(비교대상){
 *  case 조건값1 : 결과값1;
...
-- 직급코드'J6'인 사원들의 급여는 20% 인상해서 조회
-- 직급코드'J5'인 사원들의 급여는 30% 인상해서 조회
...
SELECT
  ...
  FROM
       EMPLOYEE;
-- 1. SUM(숫자 타입) : 해당 컬럼값들의 총 합계를 반환해주는 함수
-- 전체 사원의 총 급여합계
SELECT
       SUM(SALARY)
  FROM 
...
-----------------------------------------------------------------------------------------
-- 2. AVG(숫자 타입) : 해당 컬럼값들의 평균값을 구해서 반환
-- 전체사원들의 급여 평균 구하기
SELECT
       ROUND(AVG(SALARY))
  FROM 
...
-- 5. COUNT(*/ 컬럼명 / DISTINCT 컬럼명) : 행 갯수 세서 반환
...
-- 현재 사원들이 속해 있는 부서 개수
-- COUNT (DISTINCT 컬럼명) : 제시한 해당 컬럼값의 중복값이 존재할 경우 하나로만 세서 반환
SELECT 
