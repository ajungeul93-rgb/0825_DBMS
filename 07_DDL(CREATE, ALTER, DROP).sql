 ...
 *                    ,인덱스(INDEX), 패키지(PACKGE), 트리거(TRIGGER)
 ...
 * < 자료형 >
 * 
 * - 문자(CHAR(크기) / VARCHAR2(크기) / NVARCHAR2(크기)) : 크키는 BYTE단위
 *                                                     (NVARCHAR는 예외)
 *                                 숫자, 영문자, 특수문자 => 1글자당 1BYTE
 *                                 한글, 일본영, 중국어  => 1글자당 3BYTE
 * - CHAR(바이트 크기) : 최대 2000BYTE까지 지정 가능
 ...
 *                  다국어 지원 (위 두형식은 BYTE로 하지만 문자수로 표시)
...
-- 새로운 테이블을 만들면서 NOT NULL제약 조건 달아보기
...
-- NULL을 ("BJG12"."TB_USER_NOT_NULL"."USER_NO") 안에 삽입할 수 없습니다.
-- Error position: line: 145 pos: 36
...
-- 제약 조건명을 달아두면 문제가 생긴 원인은 조금 더 쉽게 유추할 수 있음       




--GENDER컬럼은 '남' 또는 '여'라는 값만 들어갈 수 있게 하고싶음
--------------------------------------------------------------------------------------
/*
 * 3.CHECK 제약조건
...
CREATE TABLE TB_USER_CHECK(
    USER_NO NUMBER NOT NULL CHECK, 
    USER_ID VARCHAR2(20) NOT NULL UNIQUE,
    USER_PWD VARCHAR2(20) NOT NULL,
    GENDER CHAR(3) CHECK(GENDER IN ('남', '여')) -- TRUE 일때만 INSERT 가능
);
...
--ALTER TABLE JOB_CODY MODIFY JOB_CODE NUMBER;
--ALTER TABLE JOB_CODY MODIFY JOB_CODE CHAR(1);
...
-- 3_1) 컬럼명 바꾸기 : ALTER TABLE 테이블명 RENAME COLUMN 기존컬럼명 TO 바꿀컬럼명
