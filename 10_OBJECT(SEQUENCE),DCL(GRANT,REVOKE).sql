 ...
 * 정수값을 자동으로 순차 증가시켜 생성해줌
 * 
 * -- 회원 번호, 사원 번호, 게시글 번호, 예약 번호, 주문 번호 등등 채번(숫자를 매기는 것)할 때 주로사용
 * 
 * 1. 시퀀스 객체 생성 구문
 * 
 * [ 표현법 ]
 ...
 *  INCREMENT BY 증가값 -> 값을 증가시킬 때 몇씩 증가할건지 지정하는 옵션
 *  MAXVALUE 최대값 -> 최대값 지정 옵션
 *  MINVALUE 최소값 -> 최소값 지정 옵션
 *  CYCLE / NOCYCLE -> 순환(반복) 여부 지정
 * ------------------- 생략 가능
 * CACHE 바이트크기 / NOCACHE -> 캐시 메모리를 쓸건지 말지
 *                         -> CACHE_SIZE 기본값은 20 BYTE                    
 *   
 * * CACHE : 미리 발생할 값을 생성해서 저장을 해둘건지 말건지 여부 지정
 *          값이 필요할 때마다 매번 새롭게 값을 생성해내는 것 보다는
 *          캐시 공간에 미리 생성된 값들을 가져다 쓰는것이 훨씬 속도적인 측면에서 이득이 있음
...
/*
 *  2. 시퀀스를 사용하는 방법
 * 시퀀스명.CURRVAL : 현재 시퀀스의 값(마지막에 성공적으로 만들어진 NEXTVAL 값)
 * 시퀀스명.NEXTVAL : 시퀀스 값을 증가시킨 뒤 증가된 시퀀스의 값
 *                  기존 시퀀스 값에서 INCREMENT BY값만큼 증가한 값
 *                  (시퀀스명.CURRVAL + INCREMENT BY값(기본값 1))
 * 
 * 시퀀스 생성 시 첫 NEXTVAL 은 START WITH로 지정한 시작 값으로 발생
 */
